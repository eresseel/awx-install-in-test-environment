...
Vagrant.configure("2") do |config|
    ...
    provision_vm = <<-SCRIPT
        apt-get update
        apt-get -y install python3-minimal python3-pip apt-transport-https ca-certificates curl gnupg-agent software-properties-common
        wget -qO- https://get.docker.com | bash
        pip3 install ansible ansible-lint docker docker-compose
        git clone -b 17.0.1 https://github.com/ansible/awx.git
        cd ./awx/installer/
        echo admin_password=admin >> inventory
    ...
    machines=[ {
        ...
        :hostname => "www.awx",
        :ip => "192.168.56.102",
        :resource_limit => [ {:cpu => 4, :memory => 8192} ],
        :shell => provision_vm,
